      ...
      <TabGroup :selectedIndex="tabIndex" @change="tabIndex = $event">
        <TabList class="grid grid-cols-2 gap-2 rounded-lg bg-zinc-50 p-1">
          <Tab v-slot="{ selected }" as="template">
            <button class="w-full rounded-md py-2 text-sm font-medium transition focus:outline-none" :class="selected
          ...
          <TabPanel>
            <form @submit.prevent="onSubmit" novalidate>
              ...
              <div class="relative">
                ...
                <input v-model.trim="totpCode" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="123456"
                  autocomplete="one-time-code" class="w-full rounded-lg border border-rose-200/60 bg-rose-100/40 placeholder:text-zinc-400
                         text-zinc-900 pl-10 pr-3 py-3 tracking-widest text-center outline-none
              ...
              <div class="mt-3 flex items-center justify-between">
                ...
                <a href="/login" class="text-xs underline text-zinc-600 hover:text-zinc-800"
              ...
              <button :disabled="loading || totpCode.length !== 6" class="mt-6 w-full rounded-full bg-rose-500 hover:bg-rose-600
          ...
          <TabPanel>
            <form @submit.prevent="onSubmit" novalidate>
              ...
              <div class="relative">
                ...
                <input v-model.trim="backupCode" type="text" placeholder="e.g. BDQMM-KSG49" class="w-full rounded-lg border border-rose-200/60 bg-rose-100/40 placeholder:text-zinc-400
                        text-zinc-900 pl-10 pr-3 py-3 outline-none focus:bg-white focus:border-rose-300" @input="backupCode = backupCode
                          .toUpperCase()
                          .replace(/[^A-Z0-9]/g, '')
                          .replace(/(.{5})/g, '$1-')
                          .replace(/-$/, '')
              ...
              <button :disabled="loading || backupCode.length < 8" class="mt-6 w-full rounded-full bg-rose-500 hover:bg-rose-600
                       disabled:opacity-60 disabled:cursor-not-allowed
                       text-white font-semibold py-3 text-sm shadow-sm transition">
                <span v-if="!loading">Continue</span>
                <span v-else class="inline-flex items-center gap-2">
