    ...
    <div class="rounded-2xl border border-zinc-200 bg-white shadow-sm p-5 md:p-6">
      ...
      <form @submit.prevent="onSubmit" novalidate>
        ...
        <p class="mt-4 text-center text-[13px] text-zinc-600">
          ...
          <a href="/login" class="underline hover:text-zinc-900" v-float-tip="'Go back to sign in.'">Sign in</a>
        </p>
      </form>
    </div>
  </section>
  ...
  methods: {
    async onSubmit() {
      ...
      try {
        ...
        var res = await axios.post("/password/reset/start", { UserName: this.form.UserName });
