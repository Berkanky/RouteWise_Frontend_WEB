<template>
  <section class="min-h-[88vh] w-full px-4 sm:px-6 py-8 sm:py-10
    ...
    <div class="w-full max-w-lg">
      ...
      <div class="px-0 sm:px-2 md:px-8 py-0 sm:py-2 md:py-8
        ...
        <form @submit.prevent="onSubmit" novalidate class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              <input :type="showPass ? 'text' : 'password'" v-model.trim="password" autocomplete="new-password"
                placeholder="••••••••" :disabled="success" class="w-full rounded-lg bg-zinc-50 placeholder:text-zinc-400 text-zinc-900 pr-10 pl-3 py-3
                       ring-1 ring-inset ring-zinc-200 focus:bg-white focus:ring-2 focus:ring-zinc-900/20
                       outline-none transition" @input="touch = true" />
              <button type="button" :disabled="success" @click="showPass = !showPass"
          ...
          <div>
            ...
            <div class="relative">
              <input :type="showConfirm ? 'text' : 'password'" v-model.trim="confirmPassword"
                autocomplete="new-password" placeholder="••••••••" :disabled="success" class="w-full rounded-lg bg-zinc-50 placeholder:text-zinc-400 text-zinc-900 pr-10 pl-3 py-3
                       ring-1 ring-inset ring-zinc-200 focus:bg-white focus:ring-2 focus:ring-zinc-900/20
                       outline-none transition" />
              <button type="button" :disabled="success" @click="showConfirm = !showConfirm"
          ...
          <!-- Submit -->
          <button v-if="!success" type="submit" :disabled="loading || !formOk || success" class="mt-2 w-full rounded-full bg-zinc-900 hover:bg-black text-white font-semibold py-3 text-sm shadow-sm
        ...
        <div v-if="success" class="mt-6 border-t border-zinc-100 pt-5">
          ...
          <div :class="[
            'grid gap-3',
            buttonsCount === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'
          ]">
            <button v-if="this.Type == 'backup_code'" type="button" v-on:click="this.go_totp_setup_page()"
            ...
            <button type="button" v-on:click="this.go_login_page();"
              class="group rounded-2xl bg-white ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 p-4 shadow-sm transition flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="h-10 w-10 rounded-xl bg-zinc-100 flex items-center justify-center">
                  <svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export default {
  ...
  computed: {
    ...
    buttonsCount() { return this.Type === 'backup_code' ? 2 : 1; }
  },
  created() {
