  ...
  <div class="min-h-screen bg-white px-3 py-6 sm:px-6">
    <div class="mx-auto max-w-7xl">
      ...
      >
        <div
          ...
          :key="route._id || idx"
          class="h-full flex flex-col bg-[#fcfcfc] ring-1 ring-zinc-100 rounded-xl shadow-sm
        ...
        >
          <div class="flex-1">
            <div class="text-right">
              <div class="inline-flex items-center gap-2">
                <span
                  class="text-[10px] uppercase bg-black text-white px-2 py-1 tracking-wide rounded"
                >
                  {{ route.TravelMode || '—' }}
                </span>
            ...
            <div class="mt-4 pb-2 mb-2 border-b border-zinc-100">
              ...
              <div class="flex flex-wrap gap-1.5">
                <span
                  v-for="(alt, i) in route.decrypted_calculated_route_polylines"
                  :key="alt._id || i"
                  class="inline-flex items-center text-[11px] px-2 py-1 rounded-full
          ...
          <div class="mt-auto pt-3 border-t border-zinc-100 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              @click="handleViewDetail(route._id)"
              aria-label="View detail for {{ route.Name }}"
      ...
      <div
        v-else
        class="flex items-center justify-center py-16 sm:py-24"
      >
        <div
          class="w-full max-w-md rounded-3xl bg-[#fcfcfc] ring-1 ring-zinc-100 shadow-[0_18px_55px_rgba(15,23,42,0.06)]
        ...
        >
          ...
          >
            ...
            >
              <path
                d="M5 21a7 7 0 0 1 7-7 7 7 0 0 1 7 7"
                stroke="currentColor"
                stroke-width="1.8"
              ...
              />
              <path
                d="M4 6.5C4.7 4 6.7 2 9.3 1.4"
                stroke="currentColor"
                stroke-width="1.4"
                stroke-linecap="round"
              />
              <path
                d="M19.7 6.5A7 7 0 0 0 14.8 2"
                stroke="currentColor"
                stroke-width="1.4"
                stroke-linecap="round"
              />
          ...
          <h2 class="text-lg sm:text-xl font-semibold text-zinc-900 tracking-tight">
            Henüz hesaplanmış bir rota yok
          </h2>
          <p class="mt-2 text-sm text-zinc-600">
            Yakın zamanda hesapladığın rotalar burada görünecek. Başlamak için yeni bir
            rota oluştur.
          </p>

          <button
            type="button"
            @click="$router.push('/start/calculate/route')"
            class="mt-6 w-full rounded-full bg-zinc-900 hover:bg-black text-white
          ...
          <p class="mt-3 text-[11px] text-zinc-500">
            Aracını seç, güzergâhını belirle, yakıt ve ücretleri anında gör.
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="bg-white/90 backdrop-blur border-t border-zinc-200/80">
        <div
          class="h-14 sm:h-[60px] px-3 sm:px-4 flex items-center justify-between gap-4
                 md:mx-auto md:max-w-3xl md:justify-center"
          style="padding-bottom: env(safe-area-inset-bottom);"
        >
          <div class="flex items-center gap-3 min-w-0">
            <div class="relative h-1.5 w-28 rounded bg-zinc-200/90 overflow-hidden">
              <div
                class="absolute inset-y-0 left-0 bg-zinc-900 transition-all duration-300"
                :style="{ width: progressPct + '%' }"
              ></div>
            </div>
            <p
              v-if="TotalCount !== null"
              class="text-[12px] text-zinc-600 truncate"
            >
          ...
          <button
            ...
            class="inline-flex h-10 sm:h-11 px-4 items-center justify-center gap-2
